package com.github.jelmerk.knn.examples.programs.airy;
...
public class airy_feeder {
    private static int[][] bd = {{-5000, 5000}};
    private static double theta = 0.000716;
    private static String DLL_PATH = "C:\\Users\\Muhammad Ashfaq\\OneDrive\\Programming\\hnswlib-master-java-internetware\\hnswlib-examples\\hnswlib-examples-java\\src\\main\\java\\com\\github\\jelmerk\\knn\\examples\\programs\\airy\\dpt_airy.dll";
    private static Sender mySender = (Sender) Native.load(DLL_PATH, Sender.class);
    public interface Sender extends Library {
        boolean Produces_Error(double n);
    }
    ...
    private static long[] randomTesting(long mySeed, BufferedWriter writer) throws IOException {
        ...
        while (true) {
            ...
            boolean revealFailure = mySender.Produces_Error(tc.vector[0]);
    ...
    private static long[] fscsTesting(long mySeed, BufferedWriter writer) throws IOException {
        Random generator = getRandomGenerator(mySeed);
        long fMeasure = 0;
        long tcGenTime = 0;
        long execTime = 0;
        ...
        while (true) {
            ...
            fMeasure++;
            double tc = tcP[(int) (fMeasure - 1)].vector[0];

            start = System.nanoTime();
            boolean revealFailure = mySender.Produces_Error(tc);
            execTime = execTime + (System.nanoTime() - start);
            if (revealFailure) {
                writer.write(fMeasure + "\t" + tcGenTime + "\t" + execTime + "\n");
                return new long[]{fMeasure, tcGenTime, execTime};
    ...
    private static long[] limBalKdfcTesting(long mySeed, BufferedWriter writer) throws IOException {
        Random generator = getRandomGenerator(mySeed);
        long tcGenTime = 0;
        long execTime = 0;
        long fMeasure = 0;
    ...
    private static long[] hnswTesting(int ef, long mySeed, BufferedWriter writer) throws InterruptedException, IOException {
        Random generator = getRandomGenerator(mySeed);
        ...
        while (true) {
            ...
            start = System.nanoTime();
            boolean revealFailure = mySender.Produces_Error(selected_set.get(selected_set.size() - 1).vector[0]);
            execTime = execTime + (System.nanoTime() - start);
            if (revealFailure) {
    ...
    private void SimRunner(int sim, boolean withSameSeed) throws InterruptedException, IOException {
        BufferedWriter RTwriter = new BufferedWriter(new FileWriter("AiryRT"));
        BufferedWriter FSCSwriter = new BufferedWriter(new FileWriter("AiryFSCS"));
        BufferedWriter LimBalKdfcwriter = new BufferedWriter(new FileWriter("AiryLimBalKdfc"));
        BufferedWriter HNSWwriter = new BufferedWriter(new FileWriter("AiryHNSW"));
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        airy_feeder myAiryFeeder = new airy_feeder();
        boolean seedOpt = false;
        myAiryFeeder.SimRunner(simulations, seedOpt);
