package com.github.jelmerk.knn.examples.programs.probks;
...
public class ProbksFeeder {
    public static double[][] bd = {{-50000, 50000}};
    public static double theta = 0.000387;
    public static String DLL_PATH = "C:\\Users\\Muhammad Ashfaq\\OneDrive\\Programming\\hnswlib-master-java\\hnswlib-examples\\hnswlib-examples-java\\src\\main\\java\\com\\github\\jelmerk\\knn\\examples\\programs\\probks\\dpt_probks.dll";

    public interface Sender extends Library {
        double probks(double n);
        double probksm(double n);
    }
    ...
    private static long[] randomTesting(long mySeed, BufferedWriter writer) throws IOException {
        ...
        while (true) {
            ...
            fMeasure++;
            double x = tc.vector[0];
            start = System.nanoTime();
            boolean revealFailure = mySender.probks(x) != mySender.probksm(x);
    ...
    private static long[] fscsTesting(long mySeed, BufferedWriter writer) throws IOException {
        ...
        while (true) {
            ...
            fMeasure++;
            double x = tcP[(int) (fMeasure - 1)].vector[0];
    ...
    private static long[] hnswTesting(int ef, long mySeed, BufferedWriter writer) throws InterruptedException, IOException {
        ...
        while (true) {
            ...
            tcGenTime = tcGenTime + (System.nanoTime() - start);
            double x = selected_set.get(selected_set.size() - 1).vector[0];
    ...
    private void SimRunner(int sim, boolean withSameSeed) throws InterruptedException, IOException {
        BufferedWriter RTwriter = new BufferedWriter(new FileWriter("ProbksRT"));
        BufferedWriter FSCSwriter = new BufferedWriter(new FileWriter("ProbksFSCS"));
        BufferedWriter LimBalKdfcwriter = new BufferedWriter(new FileWriter("ProbksLimBalKdfc"));
        BufferedWriter HNSWwriter = new BufferedWriter(new FileWriter("ProbksHNSW"));
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        ProbksFeeder myProbksFeeder = new ProbksFeeder();
        boolean seedOpt = false;
        myProbksFeeder.SimRunner(simulations, seedOpt);
        System.out.println("---Probks END---");
    }
