...
public class TestEfficiency {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		for (int[][] bd : domains) {
			...
			for (int n : num) {
				...
				String s3 = "E:/temp/" + bd.length + "d-HnswART1-" + n + ".txt";
				String s4 = "E:/temp/" + bd.length + "d-HnswART2-" + n + ".txt";

				testFscsART(s1, bd, n);
				testLimBalKDFC(s2, bd, n);
				testHnswArt(s3, bd, n, 1);
				testHnswArt(s4, bd, n, 2);
			}
		}
	}
	...
	public static void testHnswArt(String file, int[][] bd, int pointNum, int ef) throws IOException, InterruptedException {
		File f1 = new File(file);
		f1.createNewFile();
		PrintWriter out = new PrintWriter(new FileWriter(f1));
