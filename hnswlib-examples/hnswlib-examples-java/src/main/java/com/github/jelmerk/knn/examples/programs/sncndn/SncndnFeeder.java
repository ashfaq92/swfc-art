package com.github.jelmerk.knn.examples.programs.sncndn;
...
public class SncndnFeeder {
    public static double[][] bd = {{-5000, 5000}, {-5000, 5000}};
    public static double theta = 0.001623;
    public static String DLL_PATH = "C:\\Users\\Muhammad Ashfaq\\OneDrive\\Programming\\hnswlib-master-java\\hnswlib-examples\\hnswlib-examples-java\\src\\main\\java\\com\\github\\jelmerk\\knn\\examples\\programs\\sncndn\\dpt_sncndn.dll";
    ...
    private static long[] randomTesting(long mySeed, BufferedWriter writer) throws IOException {
        ...
        while (true) {
            ...
            double y = tc.vector[1];
            start = System.nanoTime();
            boolean revealFailure = mySender.Produces_Error(x, y);
    ...
    public static long[] fscsTesting(long mySeed, BufferedWriter writer) throws IOException {
        ...
        while (true) {
            ...
            double y = tcP[(int) (fMeasure - 1)].vector[1];

            start = System.nanoTime();
            boolean revealFailure = mySender.Produces_Error(x, y);            execTime = execTime + (System.nanoTime() - start);
            if (revealFailure) {
                writer.write(fMeasure + "\t" + tcGenTime + "\t" + execTime + "\n");
                return new long[]{fMeasure, tcGenTime, execTime};
            }
    ...
    public static long[] hnswTesting(int ef, long mySeed, BufferedWriter writer) throws InterruptedException, IOException {
        ...
        while (true) {
            ...
            double y = selected_set.get(selected_set.size() - 1).vector[1];
    ...
    private void SimRunner(int sim, boolean withSameSeed) throws InterruptedException, IOException {
        BufferedWriter RTwriter = new BufferedWriter(new FileWriter("SncndnRT"));
        BufferedWriter FSCSwriter = new BufferedWriter(new FileWriter("SncndnFSCS"));
        BufferedWriter LimBalKdfcwriter = new BufferedWriter(new FileWriter("SncndnLimBalKdfc"));
        BufferedWriter HNSWwriter = new BufferedWriter(new FileWriter("SncndnHNSW"));
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        System.out.println("---Sncndn START---");
        int simulations = 10000;
        SncndnFeeder mySncndnFeeder = new SncndnFeeder();
        boolean seedOpt = false;
        mySncndnFeeder.SimRunner(simulations, seedOpt);
        System.out.println("---Sncndn END---");
    }
